...
Router-based delivery for live queryset lists (ADR-0018): explicit, `db_index`-style delivery dimensions declared per field. `publish(instance)` on the writer side meets `subscribe(channel)` on the bind side; lifecycle events (create, update, delete) travel through dimension groups to connections with no prior relationship to the row. Delivery is authorization: a connection receives every event for rows whose `publish()` values intersect its `subscribe()` values — Router code reviews as security code, and every firehose is greppable (`BroadcastRouter`).
